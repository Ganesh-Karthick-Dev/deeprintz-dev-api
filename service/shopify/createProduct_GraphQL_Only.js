/**
 * MODERN SHOPIFY PRODUCT CREATION - 100% GRAPHQL (2025-10 API)
 * ============================================================
 * This file contains the updated createProduct function using ONLY GraphQL mutations.
 * No REST API calls - fully compliant with Shopify's 2025+ deprecation warnings.
 * 
 * Migration from REST API:
 * - Product creation: ✅ GraphQL productCreate
 * - Options/Variants: ✅ GraphQL productCreate with variants array
 * - Inventory: ✅ GraphQL inventorySetOnHandQuantities
 * - Images: ✅ GraphQL productCreateMedia
 * - Metafields: ✅ GraphQL metafieldsSet
 * - Publishing: ✅ GraphQL publishablePublish
 */

async function createProductGraphQLOnly(shop, accessToken, productData, shopifyService) {
  try {
    const client = shopifyService.createGraphQLClient(shop, accessToken);

    console.log('🚀 Starting 100% GraphQL product creation');
      ...
      productData.variants.forEach((variant, index) => {
        ...
        productVariants.push({
          ...
          inventoryQuantities: {
            ...
            locationId: null // Will be set after getting location
          }
        });
      });
    }
    ...
    if (productVariants.length > 0 && product.variants.edges.length > 0) {
      try {
        ...
        // Get location IDs (Canada and India)
    ...
    return {
      success: true,
      product: shopifyService.formatProductWithDetails(finalResponse.data.product),
...
module.exports = createProductGraphQLOnly;
